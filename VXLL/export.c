...
//
// Private helper function (avoids CRT dependency)
// _vscwprintf is exported from NTDLL but _scwprintf is only exported by
// C-runtime (which we don't want). Luckily it is easy to implement this
// in terms of _vscwprintf.
//
#define _scwprintf __scwprintf
static INT __scwprintf(
	IN	PCWSTR Format,
	IN	...)
{
	INT Result;
	...
	IN		BOOLEAN			LongForm)
{
	...
	while (TRUE) {
		...
		Text = (PWSTR) HeapAlloc(GetProcessHeap(), 0, TextMaxCch * sizeof(WCHAR));
