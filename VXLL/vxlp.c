	...
	OUT	PPVXLLOGENTRY		Entry)
{
	...
	*Entry = NULL;
	LogEntry = (PVXLLOGENTRY) HeapAlloc(GetProcessHeap(), 0, sizeof(VXLLOGENTRY));
	...
	Success = ReadFile(
		...
		&BytesRead,
		NULL);
	((PWSTR) LogEntry->SourceFile)[BytesRead >> 1] = '\0';
	...
	Success = ReadFile(
		...
		&BytesRead,
		NULL);
	((PWSTR) LogEntry->SourceFunction)[BytesRead >> 1] = '\0';
	...
	Success = ReadFile(
		...
		&BytesRead,
		NULL);
	((PWSTR) LogEntry->Text)[BytesRead >> 1] = '\0';
	...
	OUT		PUSHORT		SourceComponentIndex OPTIONAL)
{
	...
	*SourceComponentIndex = 0;
	...
	for (Index = 0; Index < ARRAYSIZE(LogHandle->Header.SourceComponents); ++Index) {
		if (!_wcsicmp(SourceComponent, LogHandle->Header.SourceComponents[Index])) {
