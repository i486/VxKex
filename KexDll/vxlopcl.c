...
NTSTATUS NTAPI VxlOpenLog(
	...
	IN		ULONG				CreateDisposition) PROTECTED_FUNCTION
{
	NTSTATUS Status;
	...
	try {
		...
		//
		// Find out whether we have created a new log file or we are opening an
		// existing one. If we are creating a new log file, we must create a new
		// log file header. Otherwise, we will validate the existing header.
		//
		// We can find this out through the IO status block populated by
		// NtCreateFile earlier.
		//
...
NTSTATUS NTAPI VxlCloseLog(
	IN OUT	PVXLHANDLE		LogHandle) PROTECTED_FUNCTION
{
	PVXLCONTEXT Context;
