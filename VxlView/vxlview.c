	...
	IN	LPARAM	LParam)
{
	if (Message == WM_INITDIALOG) {
		...
		SetApplicationWindow(MainWindow);
		...
		case M_EXPORT:
			...
			break;
		case M_FIND:
			SetFocus(GetDlgItem(FilterWindow, IDC_SEARCHBOX));
			break;
		case M_SELECTALL:
			FocusedWindow = GetFocus();
			GetClassName(FocusedWindow, ClassName, ARRAYSIZE(ClassName));

			if (!wcsicmp(ClassName, L"Edit")) {
				Edit_SetSel(FocusedWindow, 0, GetWindowTextLength(FocusedWindow));
			}
		case M_HELP:
			// TODO
			break;
