	...
	IN	PCWSTR	LogFileName)
{
	...
	if (NewNumberOfLogEntries == 0) {
		MessageBoxF(L"There are no entries in the log file you selected.\r\n"
					L"Please select a log file which is not empty.");
		goto OpenFailure;
	}

	NewLogEntryCache = (PPLOGENTRYCACHEENTRY) DefHeapAlloc(NewNumberOfLogEntries * sizeof(PLOGENTRYCACHEENTRY));
	if (!NewLogEntryCache) {
...
{
	//
	// We modified the log entry text in AddLogEntryToCache() (backendp.c) to shorten
	// it for the list-view (if it contains a newline). If CacheEntry->ListViewTextLength
	// is non-zero, we will now overwrite the null terminator of LogEntry->Text with a
	// '\r' character to restore the original string.
	//
	if (CacheEntry->ListViewTextLength != 0) {
		*(CacheEntry->LogEntry->Text + CacheEntry->ListViewTextLength) = '\r';
	}
}
...
{
	if (CacheEntry->ListViewTextLength != 0) {
		*(CacheEntry->LogEntry->Text + CacheEntry->ListViewTextLength) = '\0';
	}
}
