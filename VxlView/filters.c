	...
	VOID)
{
	...
	for (Index = IDC_CBFILTERCRITICAL; Index < (IDC_CBFILTERCRITICAL + LogSeverityMaximumValue); Index++) {
		...
		if (SUCCEEDED(Result)) {
			CreateToolTip(FilterWindow, Index, ToolTipText);
		}
	}
	...
	CreateToolTip(FilterWindow, IDC_SEARCHBOX, L"Search log entry message text.");
	
	CreateToolTip(FilterWindow, IDC_CBCASESENSITIVE, L"When checked, consider text with different capitalization to be different.\r\n\r\n"
													 L"For example, the search term \"Carrot\" would not match a log entry which contains \"CARROT\".");
	
	CreateToolTip(FilterWindow, IDC_CBWILDCARD, L"When checked, allow wild-card matching with * (match any sequence) and ? (match any character).\r\n\r\n"
												L"For example, the search term \"?o?ato\" would match log entries containing both \"potato\" and \"tomato\".");
	
	CreateToolTip(FilterWindow, IDC_CBINVERTSEARCH, L"When checked, only show results that do NOT match the search criteria.");

	CreateToolTip(FilterWindow, IDC_CBEXACTMATCH, L"When checked, do not show results with leading and trailing text.\r\n\r\n"
	...
	OUT	PBACKENDFILTERS	Filters)
{
	...
	TextFilter = (PWSTR) DefHeapAlloc(TextFilterBufCch * sizeof(WCHAR));
	if (TextFilter) {
		GetDlgItemText(
			FilterWindow,
			IDC_SEARCHBOX,
	...
	IN	LPARAM	LParam)
{
	...
	if (Message == WM_INITDIALOG) {
		...
		if (!wcsicmp(ClassName, L"Edit") && NotificationId != EN_CHANGE) {
