...
#define APPNAME T("KexExprt")

#ifdef _M_X64
#  define FRIENDLYAPPNAME T("Module-Definition File Generator (64 bit)")
#else
#  define FRIENDLYAPPNAME T("Module-Definition File Generator (32 bit)")
#endif
...
#define RVA_TO_VA(rva) (((LPBYTE) hMod) + (rva))
	...
	HANDLE hTempFile = CreateTempFile(T("kxp"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL);
	HANDLE hMapping;
	LPCTSTR lpszDocument;
	DWORD dwDiscard;
	...
	if (!hMod) {
		ErrorBoxF(T("Could not load the DLL file \"%s\": %s"), szDllPath, GetLastErrorAsString());
	...
	WriteFileWF(hTempFile, T("%s Generated by KexExprt (%d bit) from %s\r\n"), bPragma ? T("//") : T(";;"), BITS, szDllPath);
	...
	if (lpNtHdr->OptionalHeader.NumberOfRvaAndSizes == 0) {
		InfoBoxF(T("This DLL contains no exports."));
		goto FinishThreadProc;
	}
	...
	if (!bPragma) {
		WriteFileWF(hTempFile, T("EXPORTS\r\n"));
	}
	...
	for (i = 0; i < lpExportDir->NumberOfNames; ++i) {
		...
		if (bPragma) {
			WriteFileWF(hTempFile, T("#pragma comment(linker, \"/export:%S=%s.%S,@%I32u\")\r\n"), lpszName, szDllPath, lpszName, dwOrdinal);
		} else {
			WriteFileWF(hTempFile, T("\t%S = %s.%S @%I32u\r\n"), lpszName, szDllPath, lpszName, dwOrdinal);
		}
	...
	for (i = 0; i < lpExportDir->NumberOfFunctions; ++i) {
		...
		if (lpOrdinalHasName[i] != TRUE && lpFunctionRvas[dwOrdinal - lpExportDir->Base] != 0) {
			if (bPragma) {
				WriteFileWF(hTempFile, T("#pragma comment(linker, \"/export:__OrdinalFunction%I32X=%s.#%I32u,@%I32u,NONAME\")\r\n"), dwOrdinal, szDllPath, dwOrdinal, dwOrdinal);
			} else {
				WriteFileWF(hTempFile, T("\t__OrdinalFunction%I32X = %s.#%I32u @%I32u NONAME\r\n"), dwOrdinal, szDllPath, dwOrdinal, dwOrdinal);
			}
		}
	}
	...
	WriteFile(hTempFile, T(""), sizeof(TCHAR), &dwDiscard, NULL);
	hMapping = CreateFileMapping(hTempFile, NULL, PAGE_READONLY, 0, 0, NULL);
	lpszDocument = (LPCTSTR) MapViewOfFile(hMapping, FILE_MAP_READ, 0, 0, 0);
	...
	IN	LPARAM	lParam)
{
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == IDCANCEL) {
			...
			ofn.hwndOwner			= hWnd;
			ofn.lpstrFilter			= T("Application extension (*.dll)\0*.dll\0All Files (*.*)\0*.*\0");
