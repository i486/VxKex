	...
	IN	...)
{
	...
	UserResponse = MessageBoxV(Buttons, Icon, WindowTitle, MessageTitle, Format, ArgList);
	va_end(ArgList);
...
#else
// Stub version; in case someone decides to swap debug and release
// components around, they will still work.
KEXGDECLSPEC EXTERN_C BOOLEAN KEXGAPI ReportAssertionFailure(
	IN	PCWSTR	SourceFile,
	IN	ULONG	SourceLine,
	IN	PCWSTR	SourceFunction,
